...
function extendedData(
  ...
  entries.push(['origin', 'reconstructed']);
...
function documentAttribution(options: BuildOptions): { description: string; extended: string } {
  ...
  const lines = [
    ...
    '',
    'RECONSTRUCTED DOCUMENT. This file was generated by the TownPlanMap KML Extractor from geometry read',
    `out of ${SOURCE.name}. It is not a file published by ${SOURCE.name}, and it should not be presented`,
    'as one.',
    '',
    `Source: ${SOURCE.name}`,
  ...
  const entries: Array<[string, string]> = [
    // `origin` is stamped into every generated document so the distinction
    // between a reconstruction and the source's own file survives the file
    // leaving this tool. Original files are preserved verbatim and never pass
    // through this builder, so nothing it writes can carry any other value.
