...
export async function runExport(
  ...
  if (request.individualFiles) {
    return buildIndividualFiles(usable, locationName, documentName, skipped, notes, report);
  }
...
async function buildIndividualFiles(
  ...
  for (const group of groups) {
    ...
    for (const feature of group.features) {
      ...
      entries.push({
        path: `Individual/${buildFeatureFilename(locationName, feature.name)}`,
        content: single.kml,
      });
      written += 1;
    }
  ...
  const stem = sanitiseFilename(documentName, 'TownPlanMap_export');
  entries.unshift({ path: `KML/${stem}.kml`, content: combined.kml });
  entries.push({
    path: 'metadata.json',
